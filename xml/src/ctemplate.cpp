...
/* template_t information */

struct template_t {
    const char *filename;  /* name of template_t file */
    const char *tmplstr;   /* contents of template_t file */
    FILE *out;             /* template_t output file pointer */
    ...
    int linenum;          /* current template_t line number */
 ...
 * a template_t.  A TMPL_VAR tag can specify a format function for
...
/*
 * variables are passed to a template_t in a tree consisting of
...
/*
 * newtemplate() creates a new template_t struct and reads the template_t
 * file "filename" into memory.  If "tmplstr" is non-null then it is
 * the template_t, so we do not read "filename".
 */
    ...
    const TMPL_fmtlist *fmtlist, FILE *out, FILE *errout)
{
    ...
    if (tmplstr == 0 && filename == 0) {
        if (errout != 0) {
            fputs("C Template library: no template_t specified\n", errout);
        }
        return 0;
    }
    if (tmplstr == 0) {
    ...
    t = (template_t *) mymalloc(sizeof(*t));
...
static tagnode *
newtag(template_t *t, tag_kind kind) {
    tagnode *ret;
 ...
 * the input template_t is stored, which we free elsewhere.
 */
...
static const char *
scanspaces(template_t *t, const char *p) {
    while (*p == ' ' || *p == '\n' || *p == '\r' || *p == '\t') {
        if (*p++ == '\n') {
            t->linenum++;
        }
...
static int
scancomment(template_t *t, const char *p) {
    ...
    /* end of template_t, comment not terminated */
...
static char *
scanattr(template_t *t, const char *attrname, const char *p) {
 ...
 * or template_t tag in t->curtag or we set it to null at the end of the
 * template_t.  We start scanning at t->scanptr and when we are done,
...
scan(template_t *t) {
     ...
     * scan or else p[i] is the null at the end of the template_t.
     */
...
static tagnode *parselist(template_t *t, int stop);
 ...
 * </TMPL_IF>
 *
 * A template_t-list is any sequence (including an empty sequence)
 * of text, template_t tags, if statements or loop statements.  There
...
static void
parseloop(template_t *t, int stop) {
 ...
 * template_t-list which is any sequence of text, TMPL_VAR tags,
 ...
 * this list.  For example, if we are parsing the template_t-list
 ...
 * template_t-list that comprises the entire template_t, then "stop" is zero
 * so that we keep going to the end of the template_t.  When we are done,
 * t->curtag is the tag that caused us to stop, or null at the end of
 * the template_t.
 */

static tagnode *
parselist(template_t *t, int stop) {
 ...
 * of the parent template_t file.  If there is no directory
 * name then we strip ".../".
 */
...
/*
 * walk() walks the template_t parse tree and outputs the result.  We
 * process the tree nodes according to the data in "varlist".
 */

static void
walk(template_t *t, tagnode *tag, const TMPL_varlist *varlist) {
    const char *value;
    TMPL_loop *loop;
    TMPL_varlist *vl;
    template_t *t2;
...
/*
 * TMPL_write() outputs a template_t to open file pointer "out" using
 * variable list "varlist".  If "tmplstr" is null, then we read the
 * template_t from "filename", otherwise "tmplstr" is the template_t.
